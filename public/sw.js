...
// v99: スケーラビリティ点検(ステージング環境)で発見した日次入力保存のエラー文言を改善。
//   同じ店舗・同じ日に複数スタッフがほぼ同時に初回入力すると、後から保存した側がRLSの
//   制約(自分が作成した行のみ更新可)に阻まれ、原因不明な汎用エラー文言になっていたのを、
...
// v104: 販売用LP(/lp)を完成版へ調整。①「感覚経営から、数字経営へ。」の訴求をファースト
//   ビューのキッカー(小見出し)として復活させ、メイン見出し「売上だけでなく、利益までひと
//   目で。」の上に配置。②月次損益サマリー画像下の小さい3枠(営業利益率/人件費率/消費税
//   考慮後利益)は主役カードと情報が重複するため削除。③2枚目の画像を「目標設定フォーム」
//   から、実際の「売上」ページ(営業進捗・月間達成率・残り必要売上・月末着地予測・1日平均
//   必要売上を表示する実画面のスクリーンショット)へ差し替え、「目標管理・利益逆算」
//   セクションへ配置。旧来の重複していた月次損益表セクションは削除。既存アプリのロジック・
//   ログイン・認証・DBは無変更(lp.htmlは独立した静的ページのまま)。
// v105: 販売用LP(/lp)を最終ブラッシュアップ(訴求力・視認性・コンバージョン改善)。
//   メインコピーを「今月あといくら売れば、利益が残るか。」に変更(「感覚経営から、数字
//   経営へ。」のキッカーは維持、「売上だけでなく、利益までひと目で。」も補助コピーとして
//   残す)。悩み提示を8→6→3項目へさらに絞り込み拡大表示し、残りの悩みは複数店舗管理・
//   権限管理・機能紹介セクションへ自然に吸収。「サロン経営に必要な数字をひとつに」に
//   売上→費用→利益→目標差→残り必要売上→着地予測の価値フローを追加。POS比較の独立
//   セクションを新設(自動連携は行っていないことを明記)。権限セクションをペインファースト
//   構成に変更。料金セクションに「誰向けか」メッセージを追加。FAQに「POSとの連携は必要
//   ですか」「入力データは他社から見えませんか」を追加(解約FAQは決済/解約機能が未実装の
//   ため追加せず)。料金・CTA表記(月額1,480円/無料で試す)はLP全体で統一を再確認。既存
//   アプリのロジック・ログイン・認証・DBは無変更(lp.htmlは独立した静的ページのまま)。
// v106: 「複数店舗の数字をまとめて確認」セクションの画像を、実画面のスクリーンショット
//   (multistore.jpg)から、LP用に独自実装したコンパクトなHTML/CSSカードへ差し替え。
//   B店の営業利益・営業利益率・スタッフ生産性が「－」表示だった箇所を解消し、A店/B店/
//   会社全体サマリーいずれも具体的な数値を表示。営業利益・営業利益率は売上と同じ視覚的
//   重み(淡い青背景)で強調。カード内の余白・行間を詰めてコンパクト化(旧screenshotより
//   大幅に縦幅を圧縮)。項目構成(会社全体サマリー/店舗比較/売上構成比/店舗ランキング/
//   主要指標比較表)は既存の全店舗ビュー機能と同一、存在しない機能は追加していない。
//   既存アプリのロジック・ログイン・認証・DBは無変更(lp.htmlは独立した静的ページのまま)。
// v107: 売上入力画面の「編集」「日締め」「日締め解除」まわりの不具合を修正。①PC版で
//   「日計欄の下」にも重複して表示されていた編集/日締めボタンを解消(本来スマホ専用の
//   固定アクションバーがCSSの記述漏れでPC幅にも表示されていたのが原因)。②日締め済みの
//   店舗比較用サマリーカードで、店舗の閲覧・トグル可否を「解除できないハードロック」と
//   「日締め解除ボタンで自分から外せる一時的ロック」に分離し、締め済みの間も「編集」
//   「日締め解除」の2ボタンが表示されるようにした。③「編集」を押した直後に入力欄が編集
//   可能にならない不具合の真因(「保存」ボタンのtype="submit"→「編集」ボタンのtype=
//   "button"間でReactが同じDOMノードを使い回し、type書き換えの瞬間のクリックが誤って
//   フォーム送信を発火させていた)を特定・修正、保存ボタンをtype="button"+直接関数呼び出し
...
const CACHE_NAME = 'salon-manager-cache-v116';
const APP_SHELL = [
