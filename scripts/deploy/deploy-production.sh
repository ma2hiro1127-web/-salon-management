...
# トレーラーを、空コミット(ファイル変更なし)として追加する。
git commit --allow-empty -m "$(printf 'chore: production deploy %s\n\nDeploy-Approved-By: npm-run-deploy-production' "$(date -u +%Y-%m-%dT%H:%M:%SZ)")"
